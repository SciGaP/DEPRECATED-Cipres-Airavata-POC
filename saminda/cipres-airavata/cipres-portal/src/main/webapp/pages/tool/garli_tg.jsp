...
<h2>GARLI on XSEDE: Genetic Algorithm for Rapid Likelihood Inference - run on teragrid. (<a href="#REFERENCE">Derrick J. Zwickl (zwickl@nescent.org)</a>)</h2>
<s:form action="garli_tg" theme="simple">
...
<br/>
<A HREF="javascript:help.slidedownandjump('#streefname_userdata')">Use this Starting Tree (streefname file)</A>
...
<br/>
<A HREF="javascript:help.slidedownandjump('#constraintfile_control')">Use this Constraint File (constraintfile)</A>
...
<dt><a name=streefname_userdata><i>Use this Starting Tree (streefname file)</i></a></dt>
...
<dt><a name=constraintfile_control><i>Use this Constraint File (constraintfile)</i></a></dt>
...
alert('Please enter an integer value for Bootstrap repetitions');
return false;
}
}
// outgroup_tax
...
if (getValue('bootstrapreps_') == 0)
{
if (!confirm(messageSplit("The job will run on getValue('searchreps_value_') processors as configured. If it runs for the entire configured time, it will consume getValue('searchreps_value_') x getValue('runtime_') cpu hours")))
{
return false;
}
}
if (getValue('bootstrapreps_')!=0 && getValue('bootstrapreps_') < 32)
{
if (!confirm(messageSplit("The job will run on getValue('bootstrapreps_') processors as configured. If it runs for the entire configured time, it will consume getValue('bootstrapreps_') x getValue('runtime_') cpu hours")))
{
return false;
}
}
if (getValue('bootstrapreps_') && getValue('bootstrapreps_') >= 32)
{
...
var element = document.forms['garli_tg'].elements[parameter];
if (element == null)
return null;
