...
<title>Blast2</title>
<h2>Blast2: Blast2 searches for similarity on a selected sequence DB (<a href="#REFERENCE">Altschul, Madden, Schaeffer, Zhang, Miller, Lipman</a>)</h2>
<s:form action="blast2" theme="simple">
...
<s:select name="blast2_" list="#{ 'blastn':'blastn: nucleotide query / nucleotide db' , 'blastp':'blastp: amino acid query / protein db' , 'blastx':'blastx: nucleotide query translated / protein db' , 'tblastn':'tblastn: protein query / translated nucleotide db' , 'tblastx':'tblastx: nucleotide query transl. / transl. nucleotide db' , 'psitblastn':'psitblastn: protein query / transl. nucleotide db' }" onchange="resolveParameters()"/>
<br/>
<br/>
...
<br/>
<br/>
<A HREF="javascript:help.slidedownandjump('#nucleotid_db')">nucleotide db</A>
<font color="red" size="3">*</font>
<s:select name="nucleotid_db_" list="#{ 'NCBI_NT':'NCBI Non-Redundant (nt)' , 'GBMAM':'GBMAM: Other mammals' , 'GBBCT':'GBBCT: Bacteria' , 'GBENV':'GBENV: Environmental' , 'GBEST':'GBEST: Expressed Sequence Tags' , 'GBGSS':'GBGSS: Genome Survey Sequences' , 'GBHTC':'GBHTC: High Throughput cDNA' , 'GBHTG':'GBHTG: High Throughput Genomic' , 'GBPAT':'GBPAT: Patented' , 'GBINV':'GBINV: Invertebrates' , 'GBPHG':'GBPHG: Phages' , 'GBPLN':'GBPLN: Plants (including yeast)' , 'GBPRI':'GBPRI: Primates' , 'GBROD':'GBROD: Rodents' , 'GBSTS':'GBSTS: Sequence Tagged Sites' , 'GBSYN':'GBSYN: Synthetic' , 'GBUNA':'GBUNA: Unannotated' , 'GBVRL':'GBVRL: Virus' , 'GBVRT':'GBVRT: Other vertebrates' , 'REFSEQ_FUNGI_GENOMIC':'RefSeq Fungi: Genomic' , 'REFSEQ_FUNGI_RNA':'RefSeq Fungi: RNA' , 'REFSEQ_INV_GENOMIC':'RefSeq Invertebrate: Genomic' , 'REFSEQ_INV_RNA':'RefSeq Invertebrate: RNA' , 'REFSEQ_MICRO_GENOMIC':'RefSeq Microbial: Genomic' , 'REFSEQ_MITO_GENOMIC':'RefSeq Mitochondrion: Genomic' , 'REFSEQ_PLANT_GENOMIC':'RefSeq Plant: Genomic' , 'REFSEQ_PLANT_RNA':'RefSeq Plant: RNA' , 'REFSEQ_PLASM_GENOMIC':'RefSeq Plasmid: Genomic' , 'REFSEQ_PLASM_RNA':'RefSeq Plasmid: RNA' , 'REFSEQ_PLAST_GENOMIC':'RefSeq Plastid: Genomic' , 'REFSEQ_PLAST_RNA':'RefSeq Plastid: RNA' , 'REFSEQ_PROT_GENOMIC':'RefSeq Protozoa: Genomic' , 'REFSEQ_PROT_RNA':'RefSeq Protozoa: RNA' , 'REFSEQ_VERTM_GENOMIC':'RefSeq Mammalian: Genomic' , 'REFSEQ_VERTM_RNA':'RefSeq Mammalian: RNA' , 'REFSEQ_VERTO_GENOMIC':'RefSeq Other Vertebrate: Genomic' , 'REFSEQ_VERTO_RNA':'RefSeq Other Vertebrate: RNA Sequences' , 'REFSEQ_VIRAL_GENOMIC':'RefSeq Viral: Genomic' , 'TPA_NUCLEIC':'GENBANK: Third-Party Annotation' , 'ENSEMBL':'Ensembl' }" onchange="resolveParameters()"/>
<br/>
<br/>
...
<dd>The five BLAST programs described here perform the following tasks:</dd><dd>. blastp compares an amino acid query sequence against a protein sequence database;</dd><dd>. blastn compares a nucleotide query sequence against a nucleotide sequence database;</dd><dd>. blastx compares the six-frame conceptual translation products of a nucleotide query sequence (both strands) against a protein sequence database;</dd><dd>. tblastn compares a protein query sequence against a nucleotide sequence database dynamically translated in all six reading frames (both strands).</dd><dd>. tblastx compares the six-frame translations of a nucleotide query sequence against the six-frame translations of a nucleotide sequence database.</dd><dd>psitblastn compares a protein query sequence against a nucleotide sequence database dynamically translated in all six reading frames (both strands) using a position specific matrix created by PSI-BLAST.</dd>
...
<dt><a name=nucleotid_db><i>nucleotide db</i></a></dt>
...
</dd><dd>This options also takes a string as an argument. One may use such a string to change the specific parameters of seg or invoke other filters. Please see the 'Filtering Strings' section (below) for details.</dd><dd>
</dd>
...
<dd>
</dd><dd>The programs blastn and blastp offer fully gapped alignments. blastx and tblastn have 'in-frame' gapped alignments and use sum statistics to link alignments from different frames. tblastx provides only ungapped alignments.</dd><dd>
</dd>
...
<dd>default is 5 for blastn, 10 for blastp, blastx and tblastn</dd>
<dt><a name=extend_a_gap><i>Cost to extend a gap (-E)</i></a></dt>
<dd>Default values blastn=2, blastp, blastx and tblastn=1</dd><dd>Limited values for gap existence and extension are supported for these three programs. Some supported and suggested values are:</dd><dd>Existence Extension</dd><dd>10 1</dd><dd>10 2</dd><dd>11 1</dd><dd>8 2</dd><dd>9 2</dd><dd>(source: NCBI Blast page)</dd>
<br></br>
...
<dd>Causes NCBI gi identifiers to be shown in the output along with the accession and/or locus name. </dd><dd>Warning: available only for NCBI db's such as nrprot.</dd>
<dt><a name=seqalign_file><i>SeqAlign file (-J option must be true) (-O)</i></a></dt>
<dd>SeqAlign is in ASN.1 format, so that it can be read with NCBI tools (such as sequin). This allows one to view the results in different formats.</dd>
<br></br>
...
// nucleotid_db
if (getValue('blast2_').match("^blastn|tblast[nx]"))
...
var element = document.forms['blast2'].elements[parameter];
if (element == null)
return null;
