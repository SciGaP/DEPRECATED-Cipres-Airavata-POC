...
public class CreateTask extends ManageTasks
{
	...
	public static final String CANNOT_SUBMIT =
		...
		"this message in error, please contact customerservice@phylo.org";
		...
		if (button != null && button.length > 0) {
			if (button[0].equals(SET_DESCRIPTION)) {
				...
				if (label == null || label.equals(""))
					...
					getCurrentTask().setLabel(label);
					addActionMessage("Description \"" + label +
						"\" successfully set to current task.");
				}
			} else if (button[0].equals(SELECT_DATA)) {
				Long[] selectedIds = getSelectedIds();
				if (selectedIds == null || selectedIds.length < 1) {
					addActionError("You must select one or more data items "+
						"to set them as input to the current task.");
					setTab(SELECT_DATA);
				} else {
	...
	*/
	...
	public void setTool(String tool) {
		// if the tool changes, the parameters must be
		// cleared, but the main input must be preserved
			...
			if (tool != null) {
				...
				try {
					ToolParameters parameterAction =
						(ToolParameters)Class.forName("org.ngbw.web.actions.tool." +
							tool.toLowerCase()).newInstance();
					parameterAction.setSession(getSession());
					parameterAction.reset();
					setParameters(task, parameterAction.getToolParameters());
					setInputMap(parameterAction.getInputDataItems());
				} catch (Throwable error) {
					reportError(error, "Error pre-populating default parameter values " +
						"for tool \"" + tool.toString() + "\"");
	...
	public boolean validateTask() {
		...
		if (task == null)
			return false;
		// ensure label has been set
		String label = task.getLabel();
		if (label == null || label.trim().equals("")) {
			errors.append("Please enter a description for your task");
		}
		// ensure tool has been set
		String tool = task.getToolId();
		if (tool == null)
			errors.append("Please choose a tool for your analysis");

		// ensure input has been set
		String inputParam = getMainInputParameter(tool);

		Map<String, List<TaskInputSourceDocument>> inputMap = getInputMap();
		if (inputParam == null || inputMap == null ||
			inputMap.containsKey(inputParam) == false) {
	...
	protected Task cloneTask(Task task) {
		if (task == null)
			...
			else return session.cloneTask(task);
		} catch (Throwable error) {
	...
	protected void setParameters(Task task, Map<String, String> parameters) {
		if (task == null)
			return;
		else {
			clearParameters(task);
			if (parameters != null && parameters.size() > 0) try {
				task.toolParameters().putAll(parameters);
			} catch (Throwable error) {
				debug("Error setting parameter map of size " + parameters.size() +
...
/**/
debug("Attempting to save task \"" + task.getLabel() + "\"...");
dumpToolParameters(task);
/**/
...
/**/
debug("Checking saved task...");
Task savedTask = session.findTask(task.getTaskId());
dumpToolParameters(savedTask);
/**/
					setCurrentTask(null);
					refreshFolderTaskTabs();
					return task;
				} else throw new RuntimeException("Task failed validation.");
			}
		} catch (Throwable error) {
	...
	protected Task saveAndRunTask() throws DisabledResourceException 
	{
		try 
		{
