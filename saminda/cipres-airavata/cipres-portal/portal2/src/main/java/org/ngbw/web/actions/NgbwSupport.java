...
import org.ngbw.web.treeimage.DrawAction;
...
implements ParameterAware, SessionAware, ServletRequestAware, ServletResponseAware
{
	...
	public static final String EMAIL_HOST = "smtp.ucsd.edu";
	public static final String EMAIL_SENDER = "mmiller@sdsc.edu";
	...
	*/
	...
	public long getCPUHours() {
		Long hours = (Long)getSessionAttribute(CPU_HOURS);
		if (hours == null) // only happens if there's no session
		{
			hours = 0L;
		}
	...
	public void setWorkbenchSession(WorkbenchSession session) {
		...
		setSessionAttribute(CPU_HOURS, 0);
		String username = null;	
		try
		{
			username = session.getUsername();
			long userID = session.getUser().getUserId();
			Long hours = User.getSusForThisPeriod(userID);
			setSessionAttribute(CPU_HOURS, (hours == null ? 0 : hours));
		}
		catch(Exception e)
		{
			reportError(e, "Error retrieving cpu hours for user: " + username);
		}
	}
	...
	@SuppressWarnings("unchecked")
	protected void clearSessionAttribute(Object attribute) {
		if (attribute == null)
			return;
		else {
			Map session = getSession();
	...
	*/
	...
	protected void reportUserError(String message) {
	...
	public boolean isDrawConfigured()
	{
		return DrawAction.init();
	}
	//----------------------------------------------------------------
	...
	{
		long heapSize = Runtime.getRuntime().totalMemory() / MEG;
		long heapMaxSize = Runtime.getRuntime().maxMemory() / MEG;
		long heapFreeSize = Runtime.getRuntime().freeMemory() / MEG;
