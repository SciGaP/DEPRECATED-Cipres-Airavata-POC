...
/**/
String message = "Attempting to read output SourceDocument \"" + getTaskOutputKey() +
	"\"->\"" + getTaskOutputFile() + "\" from task \"" + getCurrentTaskLabel() +
	"\", running tool \"" + getToolLabel() +
	"\"...this output has SourceDocumentType \"" + type + "\" ";
if (workbench.canRead(type))
	message += "and can be read.";
else message += "and cannot be read.";
debug(message);
/**/
			...
			reportError(error, "Error determining whether currently selected " +
				"task output can be parsed into data records");
			return false;
		}
	}
...
/**/
String message = "Attempting to transform output SourceDocument \"" + getTaskOutputKey() +
	"\"->\"" + getTaskOutputFile() + "\" from task \"" + getCurrentTaskLabel() +
	"\", running tool \"" + getToolLabel() +
	"\"...this output has SourceDocumentType \"" + type + "\" ";
if (targetTypes != null && targetTypes.size() > 0)
	message += "and can be transformed.";
else message += "and cannot be transformed.";
debug(message);
/**/
					if (targetTypes != null && targetTypes.size() > 0)
						return true;
