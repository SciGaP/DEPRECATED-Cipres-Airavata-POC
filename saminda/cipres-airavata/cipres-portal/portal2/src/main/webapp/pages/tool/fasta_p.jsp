...
<title>Fasta P</title>
<h2>Fasta P: Find Similar PS from Databases (<a href="#REFERENCE">W. Pearson</a>)</h2>
<s:form action="fasta_p" theme="simple">
...
<font color="red" size="3">*</font>
<s:radio name="fasta_"
list="#{ 'fasta_t':'fasta: protein query vs similar db (FASTA release 3.0)' }" onclick="resolveParameters()"/>
<br/>
...
<dd>fasta - scan a protein sequence library for similar sequences</dd><dd>fasts/tfasts - compare a set of short peptide fragments (obtainded from a mass-spec analysis of a protein) against a protein or translated DNA database.</dd>
...
<dd>ktup sets the sensitivity and speed of the search. If ktup=2, similar regions in the two sequences being
compared are found by looking at pairs of aligned residues; if ktup=1, single aligned amino acids are examined.
ktup can be set to 2 or 1 for protein sequences, or from 1 to 6 for DNA sequences. The default if ktup is not
specified is 2 for proteins and 6 for DNA. 1ktup=1 should be used for oligonucleotides (DNA query length < 20).</dd>
...
<dd>Expectation value limit for displaying scores and alignments. Typically 10.0 for protein sequence
comparisons; 5.0 for fastx, and 2.0 for DNA sequence comparisons.</dd>
...
<dd>'+5/-4' are the default values for nucleotide match/mismatch, but '+3/-2' can perform better in some
cases.</dd>
...
<dd>Set the band-width used for optimization. -y 16 is the default for protein when ktup=2 and for all DNA
alignments. -y 32 is used for protein and ktup=1. For proteins, optimization slows comparison 2-fold and is
highly recommended.</dd>
<dt><a name=swalig><i>unlimited Smith-Waterman alignment for DNA (-A)</i></a></dt>
<dd>force Smith-Waterman alignment for output. Smith-Waterman is the default for protein sequences and
FASTX, but not for TFASTA or DNA comparisons with FASTA.</dd>
<dt><a name=noopt><i>no limited optimization (-o)</i></a></dt>
<dd>Turn off default optimization of all scores greater than OPTCUT. Shirt results by 'initn' scores reduces
the accuracy of statistical estimates.</dd>
...
<dd>This doubles the time required for a search, but allows accurate statistics to be estimated for
libraries comprised of a single protein family.</dd>
<br></br>
...
<dd>(MARKX) =0,1,2,3,4. Alternate display of matches and mismatches in alignments.</dd><dd>MARKX=0 uses ':','.',' ', for identities, conservative replacements, and non-conservative replacements, respectively.</dd><dd>MARKX=1 uses ' ','x', and 'X'. </dd><dd>MARKX=2 does not show the second sequence, but uses the second alignment line to display matches with
a '.' for identity, or with the mismatched residue for mismatches. MARKX=2 is useful for aligning large
numbers of similar sequences.</dd><dd>MARKX=3 writes out a file of library sequences in FASTA format. MARKX=3 should always be used with the
'SHOWALL' (-a) option, but this does not completely ensure that all of the sequences output will be aligned.</dd><dd>MARKX=4 displays a graph of the alignment of the library sequence with repect to the query sequence,
so that one can identify the regions of the query sequence that are conserved.</dd>
...
var element = document.forms['fasta_p'].elements[parameter];
if (element == null)
return null;
