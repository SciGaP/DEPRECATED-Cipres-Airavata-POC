...
<title>FastX</title>
<h2>FastX: Use Translated NS to Search PS DB for Similarity (<a href="#REFERENCE">W. Pearson</a>)</h2>
<s:form action="fastx" theme="simple">
...
<font color="red" size="3">*</font>
<s:radio name="fasta_"
list="#{ 'fastx_t':'fastx: DNA query (translated, 3 frames) vs protein db (frameshifts only between codons)' }" onclick="resolveParameters()"/>
<br/>
<hr/>
...
<dd>ktup sets the sensitivity and speed of the search. If ktup=2, similar regions in the two sequences
being compared are found by looking at pairs of aligned residues; if ktup=1, single aligned amino acids
are examined. ktup can be set to 2 or 1 for protein sequences, or from 1 to 6 for DNA sequences. The default
if ktup is not specified is 2 for proteins and 6 for DNA. 1ktup=1 should be used for oligonucleotides
(DNA query length < 20).</dd>
...
<dd>causes fasta/lfasta/plfasta to start numbering the aligned sequences starting with offset1 and offset2,
rather than 1 and 1. This is particularly useful for showing alignments of promoter regions.</dd>
<br></br>
<dt><a name=other_opt_comment><i><b>Other options</b></i></a></dt>
<dt><a name=filter><i>Lower case filtering (-S)</i></a></dt>
<dd>Treat lower-case characters in the query or library sequence as 'low-complexity' residues.
These characters are treated as 'X' during the initial scan, but are treated as normal residues during
the final alignment. Sinces statistical significance is calculated from similarity score calculated during
library search, low complexity regions will not produce statistical significant matches.</dd><dd>If a significant alignment contains low complexity regions the final score may be higher than the score
obtained during the search.</dd>
</div>
</div>
<script type="text/javascript">
...
var element = document.forms['fastx'].elements[parameter];
if (element == null)
return null;
