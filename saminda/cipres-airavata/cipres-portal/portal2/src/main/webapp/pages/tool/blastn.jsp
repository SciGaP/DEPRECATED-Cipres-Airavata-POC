...
<title>BlastN</title>
<h2>BlastN: Search DBs for Nucleotide Sequence similarity (<a href="#REFERENCE">Altschul, Madden, Schaeffer, Zhang, Miller, Lipman</a>)</h2>
<s:form action="blastn" theme="simple">
...
list="#{ 'blastn':'blastn: nucleotide query / nucleotide db' }" onclick="resolveParameters()"/>
<br/>
...
<dd>blastn compares a nucleotide query sequence against a nucleotide sequence database</dd>
...
</dd><dd>The -F option also takes a string as an argument. One may use such a string to change the specific parameters of seg or invoke other filters. Please see the 'Filtering Strings' section (below) for details.</dd><dd>
</dd>
...
<dd>Blast first searhcxes for short word pairs whose aligned score reaches at least this Threshold value (default for blastp is 11) (T in the NAR paper and in Blast 1.4)</dd>
...
<dd>-s option uses SRS for database entries retrieval links, whereas -e will use the original database site links.</dd>
<dt><a name=one_HSP_per_line><i>Draw one HSP per line in image instead of putting all HSP in one line (-l)</i></a></dt>
<dd>Useful for genome searches where there is only one sequence in the database.</dd>
...
var element = document.forms['blastn'].elements[parameter];
if (element == null)
return null;
