...
<h2>MAFFT 861 Test: Multiple alignment program for amino acid or nucleotide sequences; parallel version (<a href="#REFERENCE">Kazutaka Katoh, Kei-ichi Kuma, Hiroyuki Toh, and Takashi Miyata</a>)</h2>
<s:form action="mafft_861" theme="simple">
...
<br/>
How many sequences are in your alignment?
<s:textfield name="num_seqs_" size="10" maxlength="600" onchange="resolveParameters()"/>
<br/>
</div>
</div>
...
<br/>
<A HREF="javascript:help.slidedownandjump('#auto_analysis')">Automatically select an appropriate strategy from L-INS-i, FFT-NS-i and FFT-NS-2 (based on data size) (--auto)</A>
...
<br/>
<A HREF="javascript:help.slidedownandjump('#group2groupalg')">Select the group-to-group alignment algorithm</A>
<font color="red" size="3">+</font>
<s:radio name="group2groupalg_"
list="#{ '--alga':'alga' , '--algq':'algq' }" onclick="resolveParameters()"/>
<br/>
...
<dt><a name=auto_analysis><i>Automatically select an appropriate strategy from L-INS-i, FFT-NS-i and FFT-NS-2 (based on data size) (--auto)</i></a></dt>
<dd>When the number of sequences is > 10,000, FFT-NS-1 is selected. FFT-NS-1 is faster than the default (FFT-NS-2).
v6.860, 2011/09/17</dd>
...
<dt><a name=group2groupalg><i>Select the group-to-group alignment algorithm</i></a></dt>
<dd>Two different group-to-group algorithms, -alga and -algq, are selectable for progressive alignment options,
...
// group2groupalg
if (getValue('configure_analysis_') || getValue('analysis_type_')=="fast")
enable('group2groupalg_');
else disable('group2groupalg_');
// useFFT
if (getValue('configure_analysis_'))
enable('useFFT_');
...
// num_seqs
if (! getValue('num_seqs_')) {
alert('Please enter a value for the number of sequences in your alignment');
return false;
}
if (getValue('num_seqs_') == 0 ) {
alert('Please enter a non zero value for the number of sequences in your alignment');
return false;
}
// datatype
// auto_analysis
if (!getValue('configure_analysis_')) {
if ( getValue('num_seqs_') > 100000 && getValue('auto_analysis_')) {
alert('Sorry, MAFFT can only do -auto alignment for data sets up to 100,000 sequences');
return false;
}
}
// analysis_type
...
if (!getValue('configure_analysis_') && !getValue('auto_analysis_')) {
if ( getValue('num_seqs_') > 100000 && (getValue('fast_executables_')=="fftns168" || getValue('fast_executables_')=="fftns268" )) {
alert('Sorry, MAFFT can only do FFT-NS-1 and FFT-NS-2 alignment for data sets up to 100,000 sequences');
return false;
}
}
if (!getValue('configure_analysis_') && !getValue('auto_analysis_')) {
if (getValue('analysis_type_')=="accurate" && getValue('num_seqs_') > 6000 ) {
alert('Sorry, MAFFT can only do iterative alignment for data sets of up to 6,000 sequences');
return false;
}
}
// configure_analysis
...
var element = document.forms['mafft_861'].elements[parameter];
if (element == null)
return null;
