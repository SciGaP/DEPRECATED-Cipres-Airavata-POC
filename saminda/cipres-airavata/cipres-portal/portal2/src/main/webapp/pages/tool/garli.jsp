...
<title>GARLI</title>
<h2>GARLI: Genetic Algorithm for Rapid Likelihood Inference (<a href="#REFERENCE">Derrick J. Zwickl (zwickl@nescent.org)</a>)</h2>
<s:form action="garli" theme="simple">
...
<br/>
<hr/>
<A name=model_trees><h2><A HREF="javascript:help.slidedownandjump('#model_trees_comment')">Trees</A></H2>
...
<br/>
<A HREF="javascript:help.slidedownandjump('#streefname_userdata')">Upload this Starting Tree (streefname file)</A>
...
<br/>
<A HREF="javascript:help.slidedownandjump('#constraintfile_control')">Upload a Constraint File (constraintfile)</A>
...
<br/>
<A HREF="javascript:help.slidedownandjump('#bootstrapreps')">Bootstrap Repetitions (bootstapreps; 0 means normal search, greater than 0 means bootstrap only)</A>
...
<br></br>
<dt><a name=model_trees_comment><i><b>Trees</b></i></a></dt>
...
<dt><a name=constraintfile_control><i>Upload a Constraint File (constraintfile)</i></a></dt>
<dd>The constraint file contains any topology constraint specifications, or none if there are no constraints. With Version 0.96, backbone constraints can be uploaded.
...
// datatype_value
// geneticcode_value
if (getValue('datatype_value_')=="codon" || getValue('datatype_value_')=="codon-aminoacid")
enable('geneticcode_value_');
else disable('geneticcode_value_');
// d_ratematrix
if (getValue('datatype_value_')=="nucleotide")
...
// d_numratecats
if (getValue('datatype_value_')=="nucleotide" & getValue('d_ratehetmodel_')=="gamma")
enable('d_numratecats_');
else disable('d_numratecats_');
// p_ratematrix
if (getValue('datatype_value_')=="aminoacid" || getValue('datatype_value_')=="codon-aminoacid")
...
// p_numratecats
if (getValue('datatype_value_')=="aminoacid" | getValue('datatype_value_')=="codon-aminoacid" & getValue('p_ratehetmodel_')=="gamma" )
enable('p_numratecats_');
else disable('p_numratecats_');
// c_ratematrix
if (getValue('datatype_value_')=="codon")
...
// c_numratecats
if (getValue('datatype_value_')=="codon" && getValue('c_ratehetmodel_')=="nonsynonymous")
enable('c_numratecats_');
else disable('c_numratecats_');
// streefname_choose
// streefname_userdata
if (getValue('streefname_choose_')=="upload")
...
// attachments_val
// searchreps_value
if (getValue('searchreps_') > 4) {
alert('the value for ths parameter cannot be greater than 4');
return false;
}
// outgroup_tax
// collapsebranches_g
...
if (!getValue('inferinternalstateprobs_g_')) {
if (getValue('bootstrapreps_') < 0) {
alert('Bootstrap Repititions must be a non-negative integer.');
return false;
}
}
// resampleproportion
...
var element = document.forms['garli'].elements[parameter];
if (element == null)
return null;
