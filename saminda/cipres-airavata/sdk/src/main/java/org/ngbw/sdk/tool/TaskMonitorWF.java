...
public class TaskMonitorWF implements TaskMonitor
{
	private static final Log log = LogFactory.getLog(TaskMonitorWF.class.getName());
	protected static ToolRegistry toolRegistry = WorkbenchCIPRES.getInstance().getServiceFactory().getToolRegistry();
	private static int maxAttempts;
	private static int maxCheckJobsDays;

	static 
	{
		String tmp = WorkbenchCIPRES.getInstance().getProperties().getProperty("loadResults.retries");
		maxAttempts = Integer.parseInt(tmp);

		tmp = WorkbenchCIPRES.getInstance().getProperties().getProperty("checkjobs.maxDays");
		maxCheckJobsDays = Integer.parseInt(tmp);
	}

	private static BaseProcessWorker getProcessWorker(RunningTask rt) throws Exception
	{
	...
	private void handleFatalException(FatalTaskException fte, RunningTask rt, BaseProcessWorker pw)
	{
		try
		{
			if (pw != null)
	...
	private void handleException(Exception e, RunningTask rt, BaseProcessWorker pw)
	{
		try
		{
			String msg;
