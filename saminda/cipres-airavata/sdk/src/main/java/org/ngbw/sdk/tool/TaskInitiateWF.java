 ...
 */
public class TaskInitiateWF implements TaskInitiate
{
	private static final Log log = LogFactory.getLog(TaskInitiateWF.class);
	protected final ServiceFactory m_serviceFactory;
	private static int maxAttempts;

	static 
	{
		String tmp = WorkbenchCIPRES.getInstance().getProperties().getProperty("submitJob.retries");
		maxAttempts = Integer.parseInt(tmp);
	}

	public TaskInitiateWF(ServiceFactory serviceFactory)
	{
	...
	*/
		...
		throws DisabledResourceException, Exception
	{
		...
		catch (Exception e)
		{
			...
			new TaskMonitorWF().notifyJobComplete(task, "Error submitting job: " +  e.getMessage());
			throw e;
		}
	}
