 ...
 * 
 */
public class WorkbenchCIPRES implements Workbench{

	private static WorkbenchCIPRES SINGLETON;

	private static final Log log = LogFactory.getLog(WorkbenchCIPRES.class);
	 ...
	 * @return Workbench
	 */
	public static synchronized WorkbenchCIPRES getInstance() {
		if (SINGLETON == null)
			SINGLETON = new WorkbenchCIPRES();
		return SINGLETON;
	}
	 ...
	 * @return Workbench
	 */
	public static synchronized WorkbenchCIPRES getInstance(Resource cfg) {
		if (SINGLETON != null)
			throw new WorkbenchException(
					"A workbench instance already exists. Use getInstance() for follow up calls!");
		SINGLETON = new WorkbenchCIPRES(cfg);
		return SINGLETON;
	}
	 ...
	 * @return Workbench
	 */
	public static synchronized WorkbenchCIPRES getInstance(ServiceFactory serviceFactory) {
		if (SINGLETON != null)
			throw new WorkbenchException(
					"A workbench instance already exists. Use getInstance() for follow up calls!");
		SINGLETON = new WorkbenchCIPRES(serviceFactory);
		return SINGLETON;
	}

	protected WorkbenchCIPRES() {
		this(new Configuration().configure().buildServiceFactory());
	}

	protected WorkbenchCIPRES(Resource cfg) {
		this(new Configuration().configure(cfg).buildServiceFactory());
	}

	protected WorkbenchCIPRES(ServiceFactory factory) 
	{
		try
		{
	...
	*/
		...
		throws Exception, DisabledResourceException
	{
		...
		return (new TaskInitiateWF(serviceFactory)).queueTask(task, loggedInViaIPlant);
	}
