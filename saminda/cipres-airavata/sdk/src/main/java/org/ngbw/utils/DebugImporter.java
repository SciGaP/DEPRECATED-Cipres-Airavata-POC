...
class DebugImporter {
	...
	public static void main(String[] args)
	{
		try {
			...
			for (Iterator<Folder> folders = owner.findFolders().iterator() ; folders.hasNext() ; ) {
				Folder folder = folders.next();

				if (folder.getLabel().equals(TARGET_FOLDER_LABEL)) {
					target = folder;
			...
			if (target == null) {
				target = new Folder(owner);

				target.setLabel(TARGET_FOLDER_LABEL);
