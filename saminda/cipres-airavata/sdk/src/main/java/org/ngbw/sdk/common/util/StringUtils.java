...
public class StringUtils {
	private static Map<String, MessageDigest> MDMap = new HashMap<String, MessageDigest>();

	@SuppressWarnings("unchecked")
	private static MessageDigest getMessageDigestAlgorithm(String type) {
		MessageDigest digestAlgorithm = (MessageDigest) MDMap.get(type);
		if (digestAlgorithm == null) {
			try {
				digestAlgorithm = MessageDigest.getInstance(type);
				MDMap.put(type, digestAlgorithm);
	...
	public static String getMD5HexString(byte[] bytes) {
		...
		MessageDigest digestAlgorithm = getMessageDigestAlgorithm("MD5");
